...
>
  ...
  `--mod-progressbar-*` properties <strong>do not apply</strong>
  to `<swc-progress-bar>`. Remove or replace every
  `--mod-progressbar-*` override with the
  `--swc-linear-progress-*` equivalents below. Not every Spectrum
...
>
  ...
  `--_swc-linear-progress-*` private properties, and shadow DOM
