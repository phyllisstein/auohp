...
>
  ...
  `--mod-closebutton-*` properties <strong>do not apply</strong>
  to `<swc-close-button>`. Remove or replace every
  `--mod-closebutton-*` override with the
  `--swc-close-button-*` equivalents below.
</div>
...
>
  ...
  `--_swc-close-button-*` private properties, and shadow DOM are
