...
>
  ...
  `--mod-actionbutton-*` and
  `--spectrum-actionbutton-*` properties
  <strong>do not apply</strong> to `<swc-action-button>`. Remove
  or replace every override with the `--swc-action-button-*`
...
>
  ...
  `--_swc-action-button-*` private properties, and shadow DOM are
