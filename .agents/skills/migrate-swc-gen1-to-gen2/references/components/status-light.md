...
>
  ...
  `--mod-status-light-*` properties <strong>do not apply</strong>
  to `<swc-status-light>`. Remove or replace every
  `--mod-status-light-*` override with the
  `--swc-status-light-*` equivalents below.
</div>
...
>
  ...
  `--_swc-status-light-*` private properties, and shadow DOM are
