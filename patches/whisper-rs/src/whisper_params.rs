        ...
        O: Into<Option<F>>,
    {
        ...
        match closure.into() {
            Some(closure) => {
                ...
                // FIX: upstream uses trampoline::<F> here, but the closure
                // was type-erased to Box<dyn FnMut() -> bool> on line 635.
                // The trampoline must be monomorphized for the STORED type,
                // not the original closure type, or the pointer cast in the
                // trampoline is UB. (The progress callback does this correctly
                // on line 593.)
