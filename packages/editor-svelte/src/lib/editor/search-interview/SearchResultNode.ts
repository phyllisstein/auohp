...
// -----------------------------------------------------------------------------
// SearchResultNode --- structurally identical to TagChipNode (both MarkNode +
// setDOMUnmanaged badge + getDOMSlot().withAfter()), and deliberately not
// factored into a shared base: they diverge in styling and in what their
// decorator renders, and a shared base would couple two features that should
// not know about each other.
//
// The badge host stays a portal target (so a per-match affordance --- hit
// counter, jump-to-next, hover popover --- has somewhere to live) but it does
// not paint the highlight itself; the highlight is painted by the <mark> via
// CSS, inline, so it flows with the text and breaks correctly across lines.
// -----------------------------------------------------------------------------
