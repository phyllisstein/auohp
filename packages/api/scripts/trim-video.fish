...
# Usage: trim-video.fish [-l/--length MINUTES] input.mp4 [output.mp4]

argparse 'l/length=' -- $argv
or begin
    echo "Usage: trim-video.fish [-l/--length MINUTES] input.mp4 [output.mp4]"
    exit 1
end
...
set clip_length (test -n "$_flag_length"; and echo (math "$_flag_length * 60"); or echo 300)
...
if test $duration_int -le $clip_length
    echo "Video is shorter than"(math "$clip_length / 60")"minutes; copying as-is."
    cp $input $output
    exit 0
end
...
ffmpeg -ss $start -i $input -t $clip_length -c copy -y $output
